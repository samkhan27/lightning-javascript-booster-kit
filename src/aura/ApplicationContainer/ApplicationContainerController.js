({
	initContainerComponent: function(cmp, event, helper) { 
		var name = cmp.get("v.componentName"); if(helper.isEmpty(name)) return;
    ...
    initService: function(cmp, event, helper){
    	event.stopPropagation();
    	var params = event.getParam("params") || {};
    	if(params.type === "data") helper.initDataService(cmp).then(params.handler);
    	else helper.initMetadataService(cmp).then(params.handler);
    }
})